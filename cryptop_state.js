...
state = {
  ...
  samUpdate(model) {
    ...
    if (hasChanged.cryptos.sort) {
      sort = model.ui.currenciesCard.tabs.cryptos.sort;
      for (let i = 0; i < 4; i++) {
        if (sort.incOrder[i] == true) {
          sort.incOrder[i] = false;
        } else {
          sort.incOrder[i] = true;
        }
      }
    ...
    if (hasChanged.fiats.sort) {
      sort = model.ui.currenciesCard.tabs.fiats.sort;
      for (let i = 0; i < 3; i++) {
        if (sort.incOrder[i] == true) {
          sort.incOrder[i] = false;
        } else {
  ...
  updateCryptosFiltered(model) {
    const value_list = Object.values(this.data.cryptos.list);
    ...
    for (let i = 0; i < num_list; i++) {
      if ((value_list[i].code.toLowerCase().includes(filters.text)) || (value_list[i].name.toLowerCase().includes(filters.text))) {
        if (value_list[i].price >= filters.price) {
          cFiltered.push(value_list[i]);
        }
      }
    }
  ...
  updateFiatsFiltered(model) {
    const value_list = Object.values(this.data.fiats.list);
    const num_list = Object.keys(value_list).length;
  ...
  updateSortOrder(model, currency) {
    ...
    this.data[currency].filtered.sort((a, b) => {
      switch (typeof (a[prop])) {
        ...
        case 'string':
          return order * (a[prop].localeCompare(b[prop]));
      }
    });
